...
// const insert_code = async (req, res) => {
//   const Code_Solenoid = req.body.code_solenoid;
...
//     const check = await pool.request().input("code", sql.VarChar, code).query(`
//         USE Masterkey;
//         SELECT COUNT(*) AS count FROM Code_Solenoid_Checker WHERE Code = @code;
//       `);

//     if (check.recordset[0].count === 0) {
//       return res
//         .status(400)
//         .json({ result: "FAIL", message: "Code undified." });
//     }
...
//       .input("code", sql.VarChar, code).query(`
//         USE Masterkey;
//         INSERT INTO Code_Solenoid_tester (Code_Solenoid, Code)
//         VALUES (@Code_Solenoid, @code);
//       `);

//     res.status(200).json({ result: "OK", message: "updateed success." });
//   } catch (err) {
//     console.error("SQL Error:", err);
//     res.status(500).json({ result: "FAIL", message: "SQL Error." });
//   }
...
// app.post("/api.masterkey/update_code/:code", insert_code);
